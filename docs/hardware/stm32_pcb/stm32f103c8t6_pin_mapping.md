...
| `NRST` / `RST` | NRST | Input | TBD / verify from schematic | MCU reset | Reset/debug support | Needs verification | Confirm whether NRST is exposed on a header or test pad |
...
| 7-segment decimal point | TBD / verify from schematic | Output/TBD | HDSM-577G DP pin | Decimal point | Not planned | Needs verification | DP appears unconnected in the schematic image |
